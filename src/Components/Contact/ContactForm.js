...
function ContactForm() {
	...
	const newContactSchema = Yup.object().shape({
		...
		email: Yup.string().email('El correo es obligatorio').required('El correo es obligatorio'),
		phone: Yup.string()
			.matches(phoneValidate, 'Numero no valido')
			.required('El numero de telefono es obligatorio')
			.min(8, 'Deben ser 8 caracteres minimo'),
		message: Yup.string().max(50, 'El mensaje es muy largo').required('Debe colocar un mensaje')
	});
				...
				{({ errors, touched }) => {
					return (
						<Form>
							...
							<div>
								F<label htmlFor="phone">Teléfono:</label>
								<Field
									className="input-field"
									name="phone"
									id="phone"
